...
Uses websocket-client (synchronous), which is already in requirements.txt.
Reconnects automatically with exponential back-off so the GUI stays live
...
class WSClient:
    """
    Connects to ws://localhost:{port} (the UIBridge) and delivers messages
    to the GUI via thread-safe callbacks.

    Callbacks fire on the ws-client thread — callers must use app.after()
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, name="ws-client", daemon=True
        )
        self._thread.start()
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop:
            try:
                ...
                delay = 2.0  # reset on clean close
            except Exception as exc:
